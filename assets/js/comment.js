if (typeof(COMMENT_JS) === 'undefined') {
    ...
    function view_comment(id, post_id, page, opt, message) {
        ...
        var comment_url = ci_url + '/comment_list/lists/' + post_id + '?page=' + page;
    ...
    function view_comment_password(cmt_id, post_id) {
        var comment_url = ci_url + '/comment_list/password/' + cmt_id + '/' + post_id;
    ...
    function add_comment(f, post_id) {
        ...
        var content = '';
        $.ajax({
            url: ci_url + '/postact/filter_spam_keyword',
            type: 'post',
            data: {
                title: '',
        ...
        $.ajax({
            url: ci_url + '/comment_write/update',
            type: 'POST',
            cache: false,
    ...
    function delete_comment(cmt_id, post_id, page) {
        if (confirm("정말 삭제 하시겠습니까?\n\n삭제하신 후에는 복구가 불가능합니다.")) {
            $.ajax({
                url: ci_url + '/postact/delete_comment',
                type: 'POST',
                cache: false,
                data: {cmt_id: cmt_id, csrf_test_name: ci_csrf_hash},
                dataType: 'json',
                success: function (data) {
                    if (data.error) {
