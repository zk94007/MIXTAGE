if (typeof(SIDEVIEW_JS) === 'undefined') // 한번만 실행
{
    ...
    function getSideView(curObj, userid) {
        clickAreaCheck = true;
        $.ajax({
            url: ci_url + '/profile/sideview/' + userid,
            type: 'get',
            dataType: 'json',
            success: function (data) {
                if (data.error) {
                    ...
                    showSideView(curObj, userid, data.name, data.homepage, data.note, data.email, data.profile, data.following, data.id);
                }
            }
        });
    }

    function showSideView(curObj, userid, name, homepage, note, email, profile, following, id) {
        var sideView = new SideView('nameContextMenu', curObj, userid, name, homepage, note, email, profile, following, id);
        sideView.showLayer();
    }

    function SideView(targetObj, curObj, userid, name, homepage, note, email, profile, following, id) {
        ...
        if (ci_board) {
            if (userid) // 회원일 경우 아이디로 검색
                this.insertTail('userid', '<a href="' + ci_board_url + '/?sfield=post_userid&skeyword=' + userid + '">아이디로 검색</a>');
            else // 비회원일 경우 이름으로 검색
                this.insertTail('name', '<a href="' + ci_board_url + '/?sfield=post_nickname&skeyword=' + name + '">이름으로 검색</a>');
        }
        ...
        if (is_admin == 'super') {
            // 회원정보변경
            if (userid)
                this.insertTail('modify', '<a href="' + ci_admin_url + '/user/users/write/' + id + '" target="_blank">회원정보변경</a>');
            // 포인트내역
            if (userid)
                this.insertTail('point', '<a href="' + ci_admin_url + '/user/points?sfield=point.user_id&skeyword=' + id + '" target="_blank">포인트내역</a>');
            if (userid)
                this.insertTail('new', '<a href="' + ci_admin_url + '/board/post?sfield=user_id&skeyword=' + id + '" target="_blank">전체게시물</a>');
            if (userid)
                this.insertTail('new', '<a href="' + ci_admin_url + '/board/comment?sfield=post.user_id&skeyword=' + id + '" target="_blank">전체댓글</a>');
        }
    }
