...
class Videoplayer extends CI_Controller
{
    ...
    function get_jwplayer($file = '', $max_width = '')
    {
        ...
        if ( ! defined('JS_JWPLAYER6')) {
            ...
            $jwplayer .= '<script type="text/javascript">jwplayer.key="' . $this->CI->configlib->item('jwplayer6_key') . '";</script>' . PHP_EOL;
        }
        ...
        if ($is_type === 'audio') {
            ...
            $padding = '56.25';
            if ($cnt > 1) {
                $plist_set = $this->CI->configlib->get_device_view_type() === 'mobile' ? 'aspectratio: "16:9", listbar: { position: "right", size:150 }' : 'aspectratio: "16:9", listbar: { position: "right", size:200 }';
                $padding = '37.5';
            }
            $jwplayer .= '<script type="text/javascript">
                            jwplayer("' . $jw_id . '").setup({
