...
class Portfolio_list extends CI_Controller
{
	...
	function main()
	{
		...
		$category_id = (int) $this->CI->input->get('category_id');
		...
		$result = $this->CI->Portfolio_model->get_all_list_data($category_id, $is_collaboration);
...
/**
 * Added by Peter 2017-02-28 -----------------------------------------
 **/
        $user_photos = [];
    		if (element('col_id', $portfolio)) {
          $users = $this->CI->Collaboration_user_model->get_user_list(element('col_id', $portfolio), 'user.user_photo', element('user_id', $sessionuser));
    			if ($users) {
    				foreach ($users as $ukey => $uval) {
    					array_push($user_photos, array(
                'user_photo' => user_photo_url($uval['user_photo'])
              ));
    				}
    			}
    		}
