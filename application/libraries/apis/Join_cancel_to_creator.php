...
/**
 * Join_cancel_to_creator
 */


class Join_cancel_to_creator extends CI_Controller
{
	...
	function main()
	{
		...
		if (element('cre_type', $req) != '2') {
			$this->CI->apilib->make_error("잘못된 요청입니다..");
		}
		if (element('artist_user_id', $req) != element('user_id', $sessionuser)) {
			$this->CI->apilib->make_error("잘못된 요청입니다..");
		}
