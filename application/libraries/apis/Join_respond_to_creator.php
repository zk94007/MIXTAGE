...
/**
 * Join_respond_to_creator
 */


class Join_respond_to_creator extends CI_Controller
{
	...
	function main()
	{
		...
		if (element('cre_type', $request) != '1') {
			$this->CI->apilib->make_error("응답할 수 없는 요청입니다.");
		}
		if (element('artist_user_id', $request) != element('user_id', $sessionuser)) {
			$this->CI->apilib->make_error("회원님이 응답할 수 없는 요청입니다.");
		}
