...
class Send_login_email extends CI_Controller
{
	...
	function main()
	{
        ...
        $verify_url = site_url('mixtageverify/loginemail?user=' . element('user_userid', $userinfo) . '&code=' . $verificationcode);

		$email_title = '[믹스테이지] 로그인할 수 있도록 도와드립니다.';
		$email_content = '<table width="100%" cellpadding="0" cellspacing="0" style="border-left: 1px solid rgb(226,226,225);border-right: 1px solid rgb(226,226,225);background-color: rgb(255,255,255);border-top:10px solid #348fe2; border-bottom:5px solid #348fe2;border-collapse: collapse;"><tbody><tr><td width="200" style="padding:20px 30px;font-family: Arial,sans-serif;color: rgb(0,0,0);font-size: 14px;line-height: 20px;">믹스테이지</td><td style="font-size:12px;padding:20px 30px;font-family: Arial,sans-serif;color: rgb(0,0,0);font-size: 14px;line-height: 20px;"><span style="font-size:14px;font-weight:bold;color:rgb(0,0,0)">안녕하세요 ' . html_escape(element('user_username', $userinfo)) . '님,</span><br>계정에 다시 엑세스할 수 있는 링크를 보내드립니다.</td></tr><tr style="border-top:1px solid #e2e2e2; border-bottom:1px solid #e2e2e2;"><td colspan="2" style="padding:20px 30px;font-family: Arial,sans-serif;color: rgb(0,0,0);font-size: 14px;line-height: 20px;"><p>' . html_escape(element('user_userid', $userinfo)) . '님, 안녕하세요!</p><p>로그인과 관련하여 불편을 끼쳐드려 죄송합니다.</p><p>&nbsp;</p><p>계정에 다시 로그인할 수 있도록 도와드리겠습니다.</p><p><a href="' . $verify_url . '" target="_blank" style="font-weight:bold;">로그인하기</a></p><p>&nbsp;</p><p>감사합니다.</p></td></tr></tbody></table>';
