...
class Ask_view extends CI_Controller
{
	...
	function main()
	{
		...
		if (element('col_user_id', $collaboration) == element('user_id', $sessionuser)) {
			$this->CI->apilib->make_error("본인이 생성한 콜라보레이션이므로 접근이 불가합니다.");
		}
		...
		$this->CI->Ask_model->ask_list_update_readtime($col_id, $col_user_id, $ask_user_id, '1');
