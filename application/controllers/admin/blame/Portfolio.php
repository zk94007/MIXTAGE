...
/**
 * 관리자>Blame>Portfolio controller 입니다.
 */
class Portfolio extends MY_Controller
{
     ...
     * 페이지 이동시 필요한 정보입니다
     */
    public $pagedir = 'blame/portfolio';

    /**
     * 모델을 로딩합니다
     */
    protected $models = array('Portfolio_blame', 'Portfolio');

    /**
     * 이 컨트롤러의 메인 모델 이름입니다
     */
    protected $modelname = 'Portfolio_blame_model';
    ...
    public function index()
    {
         ...
         */
        $this->{$this->modelname}->allow_search_field = array('pbl_id', 'target_user_id', 'portfolio_blame.user_id', 'portfolio_blame.por_id', 'pbl_reason', 'por_content'); // 검색이 가능한 필드
        $this->{$this->modelname}->search_field_equal = array('pbl_id'); // 검색중 like 가 아닌 = 검색을 하는 필드
        $this->{$this->modelname}->allow_order_field = array('pbl_id'); // 정렬이 가능한 필드
         ...
         */
        $search_option = array('pbl_reason' => 'Blame reason');
    ...
    public function graph($export = '')
    {
        ...
        if ($export === 'excel') {
            ...
            header('Content-Disposition: attachment; filename=포트폴리오신고현황_' . cdate('Y_m_d') . '.xls');
