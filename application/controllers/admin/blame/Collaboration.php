...
/**
 * 관리자>Blame>Collaboration controller 입니다.
 */
class Collaboration extends MY_Controller
{
     ...
     * 페이지 이동시 필요한 정보입니다
     */
    public $pagedir = 'blame/collaboration';

    /**
     * 모델을 로딩합니다
     */
    protected $models = array('Collaboration_blame', 'Collaboration');

    /**
     * 이 컨트롤러의 메인 모델 이름입니다
     */
    protected $modelname = 'Collaboration_blame_model';
    ...
    public function index()
    {
         ...
         */
        $this->{$this->modelname}->allow_search_field = array('cbl_id', 'target_user_id', 'collaboration_blame.user_id', 'collaboration_blame.col_id', 'cbl_reason', 'col_desc'); // 검색이 가능한 필드
        $this->{$this->modelname}->search_field_equal = array('cbl_id'); // 검색중 like 가 아닌 = 검색을 하는 필드
        $this->{$this->modelname}->allow_order_field = array('cbl_id'); // 정렬이 가능한 필드
         ...
         */
        $search_option = array('cbl_reason' => 'Blame reason');
    ...
    public function graph($export = '')
    {
        ...
        if ($export === 'excel') {
            ...
            header('Content-Disposition: attachment; filename=콜라보레이션신고현황_' . cdate('Y_m_d') . '.xls');
        ...
        if ($this->input->post('chk') && is_array($this->input->post('chk'))) {
            foreach ($this->input->post('chk') as $val) {
                if ($val) {
					...
					$this->Collaboration_model->update(element('col_id', $getdata), $updatedata);
                }
            }
        }
