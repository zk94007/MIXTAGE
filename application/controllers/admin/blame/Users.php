...
/**
 * 관리자>Blame>Users controller 입니다.
 */
class Users extends MY_Controller
{
     ...
     * 페이지 이동시 필요한 정보입니다
     */
    public $pagedir = 'blame/users';

    /**
     * 모델을 로딩합니다
     */
    protected $models = array('User_blame');

    /**
     * 이 컨트롤러의 메인 모델 이름입니다
     */
    protected $modelname = 'User_blame_model';
    ...
    public function index()
    {
         ...
         */
        $this->{$this->modelname}->allow_search_field = array('ubl_id', 'target_user_id', 'user_blame.user_id', 'ubl_reason'); // 검색이 가능한 필드
        $this->{$this->modelname}->search_field_equal = array('ubl_id'); // 검색중 like 가 아닌 = 검색을 하는 필드
        $this->{$this->modelname}->allow_order_field = array('ubl_id'); // 정렬이 가능한 필드
         ...
         */
        $search_option = array('ubl_reason' => 'Blame reason');
        ...
        if ($this->input->post('chk') && is_array($this->input->post('chk'))) {
            foreach ($this->input->post('chk') as $val) {
                if ($val) {
                    ...
                    $where = array(
                        'target_user_id' => element('target_user_id', $getdata),
                    );
                    $blame_cnt = $this->{$this->modelname}->count_by($where);

					$updatedata = array(
						'user_blame' => $blame_cnt,
					);
					$this->User_model->update(element('target_user_id', $getdata), $updatedata);
                }
            }
        }
