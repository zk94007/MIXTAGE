...
/**
 * 관리자>포트폴리오>Clip controller 입니다.
 */
class Clip extends MY_Controller
{
     ...
     * 페이지 이동시 필요한 정보입니다
     */
    public $pagedir = 'portfolio/clip';

    /**
     * 모델을 로딩합니다
     */
    protected $models = array('Portfolio_clip');

    /**
     * 이 컨트롤러의 메인 모델 이름입니다
     */
    protected $modelname = 'Portfolio_clip_model';
    ...
    public function index()
    {
         ...
         */
        $this->{$this->modelname}->allow_search_field = array('clip_id', 'target_user_id', 'portfolio_clip.user_id', 'portfolio_clip.por_id', 'por_content'); // 검색이 가능한 필드
        $this->{$this->modelname}->search_field_equal = array('clip_id'); // 검색중 like 가 아닌 = 검색을 하는 필드
        $this->{$this->modelname}->allow_order_field = array('clip_id'); // 정렬이 가능한 필드
         ...
         */
        $search_option = array('clip_id' => 'Clip ID');
    ...
    public function graph($export = '')
    {
        ...
        $result = $this->{$this->modelname}->get_clip_count($datetype, $start_date, $end_date, $orderby);
        $sum_count = 0;
        $arr = array();
        $max = 0;
        ...
        if ($export === 'excel') {
            ...
            header('Content-Disposition: attachment; filename=Clip_' . cdate('Y_m_d') . '.xls');
        ...
        if ($this->input->post('chk') && is_array($this->input->post('chk'))) {
            foreach ($this->input->post('chk') as $val) {
                if ($val) {
                    ...
                    $clip_cnt = $this->{$this->modelname}->count_by($where);
